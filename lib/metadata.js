...
var Metadata = (function () {
    function Metadata($data, options, metadata) {
        ...
        this.options.container = this.$data.Container;
    ...
    Metadata.prototype.createEntitySetProperty = function (entitySetSchema, contextSchema) {
        var _this = this;
        var t = entitySetSchema.entityType;
        var prop = {
    ...
    Metadata.prototype.processMetadata = function (createdTypes) {
        ...
        types.push.apply(types, typeDefinitions.map(function (d) {
            ...
            if (d.baseType == '$data.Enum') {
                ...
                if (d.params[3] && Object.keys(d.params[3]).length > 0) {
                    Object.keys(d.params[3]).forEach(function (dp) { return dtsPart.push('        static ' + d.params[3][dp].name + ':any'); });
                }
        ...
        types.src += '});';
        types.dts += Object.keys(dtsModules).map(function (m) { return dtsModules[m].join('\n\n'); }).join('\n\n');
    ...
    Metadata.prototype._typeToTS = function (type, elementType, definition) {
        ...
        if (type == this.options.entitySetType) {
            ...
            return '{ (' + (definition.params.length > 0 ? definition.params.map(function (p) { return p.name + ': ' + _this._typeToTS(p.type, p.elementType, p); }).join(', ') : '') + '): $data.Promisable<void>; }';
        }
        else if (type == '$data.ServiceFunction') {
            return '{ (' + (definition.params.length > 0 ? definition.params.map(function (p) { return p.name + ': ' + _this._typeToTS(p.type, p.elementType, p); }).join(', ') : '') + '): $data.Promisable<' + definition.elementType + (definition.returnType == '$data.Queryable' ? '[]' : '') + '>; }';
        }
        else
            return type;
    };
